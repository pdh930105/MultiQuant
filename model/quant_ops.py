...
class QConv(nn.Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, args, stride=1, padding=0, dilation=1, groups=1,
                 bias=True, last_conv=False, first_conv=False):
    ...
    def group_activation_quantization(self, FPact_1):
        assert (self.weight_bit & 1) == 0, f'weight bit always support 2^k'
        assert (self.act_bit & 1 ) == 0, f'activation bit always support 2^k'
        tmp_list = []
    ...
    def select(self, weight, act):
        if self.first_conv: # only select output channel
            ...
            if self.weight_bit == 2 or self.weight_bit == 8:
                return weight[:index_o, :, :, :], act[:, :index_i_fir, :, :]
            elif self.weight_bit == 4 or self.weight_bit == 6:
                return weight[single_len: single_len + index_o, :, :, :],act[:, single_len: single_len + index_i_fir, :, :]
            ...
            if self.weight_bit == 2 or self.weight_bit == 8:
                return weight[:index_o, :index_i_fir, :, :], act
            elif self.weight_bit == 4 or self.weight_bit == 6:
                return weight[single_len: single_len+index_o, :index_i_fir, :, :], act
            ...
            if self.weight_bit == 2 or self.weight_bit == 8:
                return weight[:index_o, :, :, :], act
            elif self.weight_bit == 4 or self.weight_bit == 6:
                return weight[single_len: single_len+index_o, :, :, :], act

          
    def conv(self, FPweight_1, FPact_1, quantized=True):
        if self.first_conv:
    ...
    def forward(self, x):
        ...
        output = self.conv(FPweight_1, FPact_1,  quantized=True)
...
class QConv_Tra_Mulit(nn.Conv2d):
    def  __init__(self, in_channels, out_channels, kernel_size, args, stride=1, padding=0, dilation=1, groups=1,
                 bias=True, basewidth=0, expand_groups=0, 
...
def QConv3x3(args, in_planes, out_planes, stride=1, groups=1, dilation=1,
    ...
    return QConv(in_planes, out_planes, kernel_size=3, stride=stride,
                 padding=1, groups=groups, bias=False, dilation=dilation, args=args, last_conv=last_conv, first_conv=first_conv)
...
class SwitchableBatchNorm2d_Tra_Mulit(nn.Module):
    def __init__(self, out_channels, basewidth=0, last_conv=False, args=None):
...
class SwitchableBatchNorm2d(nn.Module):
    def __init__(self, out_channels, groups=-1, last_conv=False):
